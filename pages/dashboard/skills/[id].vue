...
<template>
  <div class="max-w-2xl mx-auto">
    <div class="mb-8">
      <NuxtLink to="/dashboard/skills" class="text-sm text-slate-500 hover:text-white mb-4 inline-flex items-center gap-1 transition-colors"><i class="i-tabler-arrow-left w-4 h-4"></i> Back to Skills</NuxtLink>
      <h1 class="text-2xl font-bold text-white tracking-tight">{{ isEditing ? 'Edit' : 'New' }} Skill</h1>
    </div>
    ...
    <form v-else @submit.prevent="save" class="space-y-6">
      ...
      <div class="space-y-1.5">
        ...
        <input v-model="form.url" class="w-full px-4 py-2.5 bg-[#11141C] border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-white/20 focus:ring-1 focus:ring-white/20 transition-all placeholder-slate-600" />
      </div>

      <div class="space-y-1.5">
        <label class="block text-sm font-medium text-slate-300">Icon</label>
        <div class="flex gap-3">
          <input v-model="form.icon" placeholder="skill-icons:python" class="flex-1 px-4 py-2.5 bg-[#11141C] border border-white/10 rounded-lg text-white text-sm focus:outline-none focus:border-white/20 focus:ring-1 focus:ring-white/20 transition-all placeholder-slate-600" />
      ...
      <div class="flex justify-end gap-3 pt-4 border-t border-white/5">
        <NuxtLink to="/dashboard/skills" class="px-4 py-2 text-sm font-medium text-slate-400 hover:text-white transition-colors">Cancel</NuxtLink>
