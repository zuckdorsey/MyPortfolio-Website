...
async function fetchItems() {
  loading.value = true
  try {
    const res = await $fetch<any[]>('/api/certifications')
    items.value = res || []
  } catch {
    items.value = []
  }
...
function openCreate() {
  ...
  form.value = { title: '', website: '', date: '', icon: '', badge_alt: '', description: '', skills: '' }
...
async function save() {
  ...
  try {
    if (editingItem.value) {
      await $fetch(`/api/certifications/${editingItem.value.id}`, {
        method: 'PUT',
        headers: getHeaders(),
        body: form.value
      })
...
async function confirmDelete() {
  ...
  saving.value = true
  try {
    await $fetch(`/api/certifications/${deletingItem.value.id}`, {
      method: 'DELETE',
      headers: getHeaders()
    })
...
<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h2 class="text-2xl font-bold text-neutral-900 dark:text-white">Certifications</h2>
        <p class="mt-1 text-sm text-neutral-500">Manage your certifications and badges</p>
      </div>
      <UButton icon="i-heroicons-plus" color="green" @click="openCreate">Add Certification</UButton>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-neutral-200 bg-white dark:border-neutral-700 dark:bg-neutral-800">
      ...
      <div v-else-if="items.length === 0" class="p-8 text-center text-neutral-500">No certifications found.</div>
      <table v-else class="w-full">
        <thead>
          <tr class="border-b border-neutral-200 bg-neutral-50 dark:border-neutral-700 dark:bg-neutral-800/50">
            ...
            <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-neutral-500">Skills</th>
        ...
        <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
          <tr v-for="item in items" :key="item.id" class="hover:bg-neutral-50 dark:hover:bg-neutral-700/30">
            ...
            <td class="px-4 py-3 text-sm font-medium text-neutral-900 dark:text-white">
              <a v-if="item.website" :href="item.website" target="_blank" class="hover:underline">{{ item.title }}</a>
              <span v-else>{{ item.title }}</span>
            </td>
            <td class="px-4 py-3">
              <span class="rounded-full bg-emerald-500/10 px-2 py-0.5 text-xs text-emerald-600 dark:text-emerald-400">
                {{ item.date || '—' }}
              </span>
            </td>
            <td class="px-4 py-3 text-sm text-neutral-500">{{ item.skills || '—' }}</td>
      ...
      <div class="p-6">
        ...
        <div class="space-y-4">
          <UFormGroup label="Title *" required>
            <UInput v-model="form.title" placeholder="Certification name" />
          </UFormGroup>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <UFormGroup label="Badge Alt Text">
              <UInput v-model="form.badge_alt" placeholder="Badge description" />
            </UFormGroup>
          </div>

          <UFormGroup label="Skills">
            <UInput v-model="form.skills" placeholder="Related skills (comma separated)" />
          </UFormGroup>

          <UFormGroup label="Description">
            <UTextarea v-model="form.description" :rows="4" placeholder="Certification description..." />
          </UFormGroup>
    ...
    <UModal v-model="showDeleteModal">
      <div class="p-6">
        <h3 class="mb-2 text-lg font-semibold text-neutral-900 dark:text-white">Delete Certification</h3>
